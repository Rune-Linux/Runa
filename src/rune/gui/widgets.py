...
class PackageRow(Gtk.ListBoxRow):
    def __init__(self, package: AURPackage):
        super().__init__()
        ...
        version_label = Gtk.Label(label=package.version)
        ...
        votes_label.set_markup(f"<small>Votes: {package.votes}</small>")
        stats_box.pack_start(votes_label, False, False, 0)
        
        pop_label = Gtk.Label()
        pop_label.set_markup(f"<small>Popularity: {package.popularity:.2f}</small>")
        stats_box.pack_start(pop_label, False, False, 0)
        
        maintainer = package.maintainer or "orphan"
        maint_label = Gtk.Label()
        maint_label.set_markup(f"<small>Maintainer: {GLib.markup_escape_text(maintainer)}</small>")
